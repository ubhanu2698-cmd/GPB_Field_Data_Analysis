    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          # ---------------- Left: Controls ----------------
          box(
            title = "Upload & Options (RAW)",
            width = 4, solidHeader = TRUE, status = "primary",
            
            # Data
            fileInput(
              "anova_file", "Upload CSV / Excel",
              accept = c(".csv", ".xlsx")
            ),
            ...
            # Alpha-Lattice options (rendered from server)
            uiOutput("alpha_block_ui"),
            
            # Trial axes
            radioButtons(
              "trial_type", "Trial Type",
              choices  = c("Single Environment" = "single_env",
                           "Multi-Environment"  = "multi_env"),
              selected = "single_env"
            ),
            radioButtons(
              "year_type", "Year Type",
              choices  = c("Single Year" = "single_year",
...
server <- function(input, output, session) {
  ...
  .fit_anova_by_design <- function(df, trait, design) {
    ...
    output$alpha_block_ui <- renderUI({
      req(input$exp_design)
      if (identical(input$exp_design, "Alpha-Lattice")) {
        tagList(
          radioButtons(
            "alpha_block_type", "Block Type (Alpha-Lattice)",
            choices  = c("Incomplete" = "incomplete", "Complete" = "complete"),
            selected = isolate(input$alpha_block_type %||% "incomplete")
          ),
    ...
    if (design == "Alpha-Lattice") {
      # Pull mapped columns (with fallbacks)
      Treatment <- input$anova_gen   %||% NULL
      Genotype  <- input$anova_gen   %||% NULL
      Rep       <- if (nzchar(input$anova_rep))   input$anova_rep   else .pick(df, "Replication") %||% .pick(df, "Rep") %||% .pick(df, "R")
      Block     <- if (nzchar(input$anova_block)) input$anova_block else .pick(df, "Block") %||% .pick(df, "Blk") %||% .pick(df, "B")
      Loc       <- if (nzchar(input$anova_env))   input$anova_env   else .pick(df, "Environment") %||% .pick(df, "Location") %||% .pick(df, "Loc") %||% .pick(df, "E")
      Year      <- if (nzchar(input$anova_year))  input$anova_year  else .pick(df, "Year") %||% .pick(df, "Y")
      ...
      # Detect available environment axes (≥2 distinct levels to matter)
      hasLoc  <- !is.null(Loc)  && Loc  %in% names(df) && length(unique(stats::na.omit(if (is.factor(df[[Loc]])) as.character(droplevels(df[[Loc]])) else df[[Loc]])))   >= 2
      hasYear <- !is.null(Year) && Year %in% names(df) && length(unique(stats::na.omit(if (is.factor(df[[Year]])) as.character(droplevels(df[[Year]])) else df[[Year]]))) >= 2
      env_cols <- c(if (hasYear) Year, if (hasLoc) Loc)   # e.g., c("Year","Location"), or single, or empty
      
      # Keep only columns we need; drop NAs and empty levels
      used <- unique(c(trait, Genotype, Rep, Block, env_cols))
      dat  <- .prepare_anova_data(df, used, trait)
      ...
      if (hasYear) dat[[Year]] <- as.factor(dat[[Year]])
      if (hasLoc)  dat[[Loc]]  <- as.factor(dat[[Loc]])
      
      # --- helpers to build formula text ----
      .env_main_term <- function() {
        if (hasYear && hasLoc) paste0(Year, " + ", Loc, " + ", Year, ":", Loc)
        else if (hasYear) Year
        else if (hasLoc)  Loc
        else ""
      }
      .nest_in_env <- function(x) {                 # prefix with Year/Loc if present
        if (length(env_cols) == 0) x else paste(paste(env_cols, collapse=":"), x, sep=":")
      }
      .g_by_env <- function() {
        if (hasYear && hasLoc) paste0(Genotype, ":", Year, " + ",
                                      Genotype, ":", Loc,  " + ",
                                      Genotype, ":", Year, ":", Loc)
        else if (hasYear) paste0(Genotype, ":", Year)
        else if (hasLoc)  paste0(Genotype, ":", Loc)
        else ""
      }
      
      env_main   <- .env_main_term()                        # "", "Year", "Loc", or "Year + Loc + Year:Loc"
      rep_term   <- .nest_in_env(Rep)                       # "Rep" or "Year:Rep" or "Year:Loc:Rep"
      block_term <- .nest_in_env(paste0(Rep, ":", Block))   # "Rep:Block" or "Year:Rep:Block" etc.
      gxe_term   <- .g_by_env()
      
      # Warn if blocks are irregular inside Env:Rep (helpful for alpha checks)
      # (non-fatal; shows a notification if available)
      if (requireNamespace("dplyr", quietly = TRUE)) {
        try({
          grp <- c(env_cols, Rep)
          n_per_blk <- dplyr::count(dat, dplyr::across(dplyr::all_of(c(grp, Block))))
          n_per_er  <- dplyr::count(n_per_blk, dplyr::across(dplyr::all_of(grp)), wt = n, name = "plots",
                                    sort = FALSE)
          irr <- dplyr::summarise(n_per_blk, .by = dplyr::across(dplyr::all_of(grp)),
                                  sd_size = sd(n, na.rm = TRUE))
          if (any(is.finite(irr$sd_size) & irr$sd_size > 0)) {
            if (exists("showNotification", inherits = TRUE)) 
              showNotification("Alpha-Lattice: unequal block sizes within some Env:Rep.", type = "warning")
          }
        }, silent = TRUE)
      }
      
      blk_type <- input$alpha_block_type %||% "incomplete"
      
      if (identical(blk_type, "incomplete")) {
        # ===== Incomplete (true alpha-lattice): Mixed model =====
        # Fixed: Genotype + (G×E if E exists) + optional Env mains (for means/reporting)
        fixed_terms <- c(env_main, Genotype, gxe_term)
        fixed_terms <- fixed_terms[fixed_terms != ""]
        fixed_txt   <- if (length(fixed_terms)) paste(fixed_terms, collapse = " + ") else "1"
        # Random: (1|Env:Rep) + (1|Env:Rep:Block)
        rand_txt    <- paste0("(1|", rep_term, ") + (1|", block_term, ")")
        fml_txt     <- paste0(trait, " ~ ", fixed_txt, " + ", rand_txt)
        fml         <- as.formula(fml_txt)
        
        # Fit with lme4 (lmerTest optional for p-values)
        validate(need(requireNamespace("lme4", quietly = TRUE),
                      "Package 'lme4' is required for Incomplete (alpha) lattice. Please install it."))
        fit <- lme4::lmer(fml, data = dat)
        return(list(kind = "lmer", fit = fit, formula = fml))
      } else {
        # ===== Complete blocks: Classical ANOVA =====
        rhs_terms <- c(env_main, rep_term, block_term, Genotype, gxe_term)
        rhs_terms <- rhs_terms[rhs_terms != ""]
        fml <- as.formula(paste0(trait, " ~ ", paste(rhs_terms, collapse = " + ")))
        fit <- aov(fml, data = dat)
        return(list(kind = "aov", fit = fit, formula = fml))
      }
    }
